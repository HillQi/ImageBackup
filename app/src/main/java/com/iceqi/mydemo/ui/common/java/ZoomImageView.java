        ...
        gestureDetector = new GestureDetector(context, new GestureDetector.SimpleOnGestureListener() {
            ...
            public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
                ...
                if (startX != maxX || startY != maxY) {
                    ...
                    //出图片的惯性滑动
                    android.widget.OverScroller os = null;
                    scroller.setFriction(ViewConfiguration.getScrollFriction());
                    scroller.fling(startX, startY, vX, vY, 0, maxX, 0, maxY);
                }
                ...
                translationAnimation.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator animation) {
                        if (scroller.computeScrollOffset()) {
                            //获得当前的x坐标
                            int dx = scroller.getCurrX() - mCurrentX;
                            mCurrentX = scroller.getCurrX();
                            //获得当前的y坐标
                            int dy = scroller.getCurrY() - mCurrentY;
                            mCurrentY = scroller.getCurrY();

                            // on Huawei P30 pro speed different between fling left & right or up & down.
                            if(dx < 0)
                                dx *= 2.3;
                            if(dy < 0)
                                dy *= 2.3;
                            ...
                            //进行平移操作
                            // disable this for do not find any difference
//                            if (dx != 0 && dy != 0)
            ...
            void output(Object msg){
                System.out.println("mine:: " + msg.toString());
            }
